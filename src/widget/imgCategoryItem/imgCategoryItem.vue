 ...
 * @Last Modified time: 2019-03-11 16:12:20
 */
// 图片空间项
<template>
  <div class="img-space-item" @click="click">
    ...
    <div class="left img-category-children" v-if="data.children.length > 0 && showChild">
      <imgCategoryItem v-for="item,index in data.children" :data="item" :key="index">
      </imgCategoryItem>
    </div>
    <!--{{ data }}-->
  </div>
...
export default {
  ...
  computed: {
    pictureCategoryId (){
      return this.$store.state.imageSpace.pictureCategoryId
    }
  },
  methods: {
    click(){
    ...
    selectCategory(id, name) {
      // alert(id);
      this.$store.state.imageSpace.pictureCategoryId = id;
      this.$store.state.imageSpace.pictureCategoryName = name;
